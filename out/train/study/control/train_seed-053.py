...
OUT = Path("out/train/study/seed-053"); OUT.mkdir(parents=True, exist_ok=True)
...
model = FastLanguageModel.get_peft_model(
    model, r=R, lora_alpha=ALPHA, lora_dropout=0.0, bias="none", random_state=53,
...
trainer = SFTTrainer(model=model, train_dataset=ds, processing_class=tok,
                     args=SFTConfig(per_device_train_batch_size=1, gradient_accumulation_steps=4,
                                    num_train_epochs=3, learning_rate=2e-4, logging_steps=10, seed=53,
...
(OUT / "training_receipt.json").write_text(json.dumps(
    {"schema": "szl.training/v1", "commit": HEAD, "state": "MEASURED", "base_model": BASE,
     ...
     "hyperparameters": {"r": R, "alpha": ALPHA, "dropout": 0.0, "epochs": 3, "lr": 2e-4, "seed": 53,
